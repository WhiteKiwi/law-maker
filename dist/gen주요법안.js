...
const util_law_maker_1 = require("./util-law-maker");
async function main() {
    const lawMakers = await (0, util_law_maker_1.loadLawMakers)();
    for (const lawMaker of lawMakers) {
        await (0, util_law_maker_1.patchLawMaker)(lawMaker.id, (lawMaker) => {
    ...
    for (const 의안번호 of 의안) {
        ...
        for (const 찬성 of 법안표결.찬성) {
            lawMakers
                ...
                await (0, util_law_maker_1.patchLawMaker)(lawMaker.id, (lawMaker) => {
                    lawMaker.주요법안표결.find((v) => v.의안번호 === 의안번호).찬반여부 = "찬성";
                    return lawMaker;
                });
            });
        }
        for (const 반대 of 법안표결.반대) {
            lawMakers
                ...
                .forEach(async (lawMaker) => {
                await (0, util_law_maker_1.patchLawMaker)(lawMaker.id, (lawMaker) => {
                    lawMaker.주요법안표결.find((v) => v.의안번호 === 의안번호).찬반여부 = "반대";
                    return lawMaker;
                });
            });
        }
        for (const 기권 of 법안표결.기권) {
            lawMakers
                ...
                .forEach(async (lawMaker) => {
                await (0, util_law_maker_1.patchLawMaker)(lawMaker.id, (lawMaker) => {
                    lawMaker.주요법안표결.find((v) => v.의안번호 === 의안번호).찬반여부 = "기권";
                    return lawMaker;
                });
            });
        }
...
async function get주요법안표결(의안번호) {
    ...
    const res = await axios_1.default.post("https://www.assembly.go.kr/portal/cnts/cntsBill/findBpmBillVoteDtl.json", `billNo=${의안번호}&_csrf=2ad09d1d-821a-4122-a9a6-a30fc9ad7558`, {
        headers: {
            ...
            Referer: "https://www.assembly.go.kr/portal/cnts/cntsCont/dataB.do?menuNo=600232&pageIndex=1&cntsDivCd=BILL&fromAge=21&toAge=21&billKindCd=&committeeId=&billNo=2119142&billName=&billNameMb=&fromAgeMb=21&toAgeMb=21&billKindCdMb=&committeeIdMb=&billNoMb=2119142",
    ...
    console.log(res.data.voteDtlList
        .filter((v) => v.hgNm === "이수진" || v.hgNm === "김병욱")
        .map((v) => ({
        이름: v.hgNm,
        정당: v.polyNm,
        찬반: v.resultVoteMod,
    ...
    return {
        ...
        찬성: res.data.voteDtlList
            .filter((v) => v.resultVoteMod === "찬성")
            .map((v) => v.hgNm),
        반대: res.data.voteDtlList
            .filter((v) => v.resultVoteMod === "반대")
            .map((v) => v.hgNm),
        기권: res.data.voteDtlList
            .filter((v) => v.resultVoteMod === "기권")
            .map((v) => v.hgNm),
    };
}
main();
